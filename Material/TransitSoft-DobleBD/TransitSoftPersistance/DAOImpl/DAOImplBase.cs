...
namespace TransitSoft.DAOImpl
{
    public abstract class DAOImplBase
    {
        ...
        // Nueva propiedad para indicar qué BD usar
        protected bool usarBaseDeDatos2;

        // Constructor original (mantiene compatibilidad)
        public DAOImplBase(String nombreDeTabla) : this(nombreDeTabla, false)
        {
        }

        // Nuevo constructor con opción de BD
        public DAOImplBase(String nombreDeTabla, bool usarBaseDeDatos2)
        {
            this.nombreDeTabla = nombreDeTabla;
            this.retornarLlavePrimaria = false;
            this.mostrarSentenciaSQL = true;
            this.usarTransaccion = true;
            this.usarBaseDeDatos2 = usarBaseDeDatos2;
        ...
        // Método helper para obtener la instancia correcta
        protected DBManager ObtenerDBManager()
        {
            return this.usarBaseDeDatos2 ? DBManager.Instance2 : DBManager.Instance;
        }

        protected void AbrirConexion()
        {
            this.conexion = this.ObtenerDBManager().Connection;
            this.conexion.Open();
        ...
        private int Ejecuta_DML(TipoOperacionDML tipoOperacion)
        {
            ...
            try
            {
                ...
                this.comando = this.ObtenerDBManager().CrearComando();
                string sql = "";
        ...
        {
            using (DbConnection conn = this.ObtenerDBManager().Connection)
            {
                ...
                {
                    conn.Open();
                    if (añadirParametro != null)
                        añadirParametro(comando, modelo);
