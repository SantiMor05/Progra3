...
public abstract class DAOImplBase {
    ...
    // Nueva propiedad para indicar qué BD usar (1, 2 o 3)
    protected int baseDeDatosDestino;

    // Constructor original (mantiene compatibilidad con código existente - usa BD 1)
    public DAOImplBase(String nombre_tabla) {
        this(nombre_tabla, 1); // Por defecto usa BD Principal
    }

    // Nuevo constructor con opción de BD
    public DAOImplBase(String nombre_tabla, int baseDeDatosDestino) {
        this.nombre_tabla = nombre_tabla;
        this.retornarLlavePrimaria = false;
        this.baseDeDatosDestino = baseDeDatosDestino;
    ...
    // Método helper para obtener la instancia correcta de DBManager según BD destino
    protected DBManager obtenerDBManager() {
        switch (this.baseDeDatosDestino) {
            case 2:
                return DBManager.getInstance2();
            case 3:
                return DBManager.getInstance3();
            default:
    ...
    protected void abrirConexion() {
        // Usa el DBManager correcto según baseDeDatosDestino
        this.conexion = this.obtenerDBManager().getConnection();
    }
    ...
    public Integer retornarUltimoAutoGenerado() {
        ...
        try {
            // CAMBIO IMPORTANTE: Usar el SQL correcto según el motor de la BD destino
            // Cada motor tiene su propia sintaxis para obtener el último ID
            String sql = this.obtenerDBManager().retornarSQLParaUltimoAutoGenerado();
            this.statement = this.conexion.prepareCall(sql);
            this.resultSet = this.statement.executeQuery();
    ...
    public Integer ejecutarProcedimientoAlmacenado(String sql,
            Boolean conTransaccion) {
        Consumer incluirValorDeParametros = null;
        Object parametros = null;
        return this.ejecutarProcedimientoAlmacenado(sql, incluirValorDeParametros, parametros, conTransaccion);
    }
